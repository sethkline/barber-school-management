...
export const studentService = {
  ...
  async listStudentDocuments(studentId: string): Promise<StudentDocument[]> {
    ...
    if (error) {
      throw new Error(
        `Failed to fetch documents for student ${studentId}: ${error.message}`
      )
    }
    return data ?? []
  },
  ...
  async uploadStudentDocument(
    studentId: string,
    documentData: StudentDocumentInsert
    ...
    if (error) {
      throw new Error(
        `Failed to upload document for student ${studentId}: ${error.message}`
      )
    }
    return data!
  },
  ...
  async removeStudentDocument(
    ...
    if (error) {
      throw new Error(
        `Failed to delete document ${documentId} for student ${studentId}: ${error.message}`
      )
    }
    return data!
  }
